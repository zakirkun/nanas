...
5. **Configure the read allowlist (optional)**: use the Database → **Catalog** tab, or call `GET /v1/projects/:pid/settings/data-allowlist` and `PATCH /v1/projects/:pid/settings/data-allowlist` with `{ "tables": ["table_name"] }` (developer role).
6. **Upload a function artifact**: call `POST /v1/projects/:pid/functions/:fid/artifacts/presign`, then `PUT` the returned API-relative upload URL with `Authorization: Bearer …` (the backend proxies to object storage). Alternatively upload via `PUT /v1/projects/:pid/functions/:fid/artifacts/upload?version=`. Register the version with `POST /v1/projects/:pid/functions/:fid/versions` using `artifact_key`, `checksum`, and `runtime`.
